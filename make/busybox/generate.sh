...
[ -z "$1" ] && rm -f $BBDIR/*.?_?? && for x in $(sed -rn 's/^\$\(PKG\)_HASH_([0-9\.]{6,8}*):=.*/\1/p' $BBDIR/busybox.mk); do $0 $x; done && exit
...
echo -n " patching ..."
[ -d "$BBDIR/busybox-$BBVER/" ] && VERZ="busybox-$BBVER" || VERZ="busybox"
cd "$BBDIR/$VERZ/"
...
echo -n " building ..."
FREETZ_GENERATE_CONFIG_IN_ONLY=y ./scripts/gen_build_files.sh "$BBDIR/$VERZ/" "$BBDIR/$VERZ/" >/dev/null
...
# ensure only BASH_IS_ASH could be selected - except avm uses a gnu-bash
depends_on BASH_IS_HUSH "FREETZ_DISABLE_OPTION_BY_MAKING_IT_DEPEND_ON_NONEXISTING_SYMBOL"
depends_on BASH_IS_NONE "FREETZ_AVM_HAS_BASH_FILE"
depends_on BASH_IS_ASH "!FREETZ_AVM_HAS_BASH_FILE"
